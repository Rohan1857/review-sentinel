...
=== "GitHub (PAT)"
    ...
    uv run review-sentinel review myorg/myrepo#123
    ```
...
=== "GitLab"
    ...
    uv run review-sentinel review mygroup/myproject#10 --platform gitlab
    ```
...
```bash
uv run review-sentinel review myorg/myrepo#123 --dry-run --prompt "focus on error handling"
```

### Model Override

```bash
uv run review-sentinel review myorg/myrepo#42 --model claude-sonnet-4-6
```
