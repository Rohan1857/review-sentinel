...
ReviewSentinel uses a **notes-based compaction strategy** for agents with notes files (the reviewer and explore sub-agents). Instead of generating a summary at compaction time (which would require an extra LLM call), the agent writes structured findings to a markdown notes file throughout execution via the `WriteNotes` tool. When compaction triggers, the notes file content is used directly as the summary.
