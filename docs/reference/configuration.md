...
ReviewSentinel supports two configuration methods: a **YAML config file** (recommended for webhook and Kubernetes deployments) and **environment variables** (for secrets, CI-provided values, and simple setups). Both can be used together — environment variables always override the YAML file.
...
| Per-repo `.review_sentinel/` files | Runtime | Override global prompt/guidelines per-repository |
...
Repositories can override the global guidelines by placing files in a `.review_sentinel/` directory at the repository root. Per-repo overrides take priority over the built-in defaults.

### General guidelines

A `.review_sentinel/guidelines.md` file replaces the global coding guidelines for that repository. When present, the built-in guidelines are **not** appended — the repo file is used exclusively.

### Language-specific guidelines

A `.review_sentinel/languages/{language}.md` file (e.g. `.review_sentinel/languages/python.md`) replaces the built-in language guideline for that language.
...
Filter events based on tags in the PR/MR title. Tags are substrings enclosed in square brackets (e.g. `[review_sentinelbot]`).
...
=== "Env var"
    ...
    ```bash
    PR_TITLE_INCLUDE_TAGS=review_sentinelbot
...
|---|---|---|---|
| `feat: add login [review_sentinelbot]` | `review_sentinelbot` | — | Processed |
| `feat: add login` | `review_sentinelbot` | — | Skipped |
| `feat: add login [skip]` | — | `skip` | Skipped |
| `feat: add login [review_sentinelbot] [skip]` | `review_sentinelbot` | `skip` | Skipped (exclude wins) |
