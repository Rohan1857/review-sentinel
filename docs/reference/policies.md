...
ReviewSentinel separates webhook event handling into two frozen Pydantic models: **FilteringPolicy** (which events to process) and **RoutingPolicy** (how to dispatch them). Both live in `review_sentinel.config.policies` and are composed into the top-level `Config`.
...
```python
from review_sentinel.config.policies import FilteringPolicy

filtering = FilteringPolicy(
    allowed_users=frozenset({"alice", "bob"}),
    allowed_repos=frozenset({"owner/repo-a"}),
    pr_title_include_tags=frozenset({"review_sentinelbot"}),
...
```python
from review_sentinel.commands.webhook.main import filter_event
...
```python
from review_sentinel.commands.webhook.main import dispatch_lifecycle_event
...
```python
from review_sentinel.commands.webhook.main import dispatch_comment_event
...
```python
from review_sentinel.commands.webhook.jobs.runner import build_runner
