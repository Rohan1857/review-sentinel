...
ReviewSentinel can be deployed as a **standalone server** or on **Kubernetes**. Both run the same webhook server — the difference is how review jobs are executed and how the server is managed.
...
| `make -C deploy teardown` | Delete the `review-sentinel` K8s namespace |
...
0 3 * * * find /tmp/review-sentinel -maxdepth 3 -name 'pr-*' -type d -mtime +7 -exec rm -rf {} +
```
