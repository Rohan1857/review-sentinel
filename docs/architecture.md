...
```
review-sentinel review owner/repo#42 [--dry-run] [--prompt "..."]
        │
        ▼
...
ReviewSentinel supports two agent execution backends. The mode is selected automatically based on the execution context.
...
|---|---|---|
| CI (`review-sentinel ci`) | LLM provider API | `ApiAgentConfig` |
| CLI (`review-sentinel review`) | Claude Code CLI | `CliAgentConfig` |
...
3. Repository guidelines (`.review_sentinel/guidelines.md` or built-in) — wrapped in `<repo-guidelines>` tags
...
System prompt composition is handled by `agent/prompts.py`, which loads guidelines from the `.review_sentinel/` directory with per-repo and per-language overrides. Language detection is based on file extensions in the PR diff.
...
- **Pricing data** — a bundled `llm/data/pricing.json` file maps model IDs to per-token rates (input, output, cache write, cache read). Generated from the [LiteLLM community pricing database](https://github.com/BerriAI/litellm/blob/main/model_prices_and_context_window.json) by running `scripts/update_pricing.py` manually.
...
3. The Job pod runs `review-sentinel run-job`, performs the review, and publishes a completion signal to `nc:job:{job_name}:done` via Redis pub/sub.
