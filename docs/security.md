...
This page covers the security model, threat surface, and hardening recommendations for ReviewSentinel — with particular focus on LLM-related risks.
...
| Per-repo overrides | `.review_sentinel/guidelines.md`, `.review_sentinel/languages/{lang}.md` | **Semi-trusted** — committed by repo maintainers, injected into the system prompt |
...
4. **Via `.review_sentinel/guidelines.md`** — per-repo guideline overrides are injected directly into the system prompt. A compromised or malicious guideline file can alter the agent's review behavior.
...
- **Review `.review_sentinel/guidelines.md` changes carefully** — these files are injected into the system prompt. Treat changes to them with the same scrutiny as CI configuration changes.
...
The `build_sanitized_env()` function in `review_sentinel/agent/sandbox.py` filters `os.environ` using an **allowlist** of safe variable names:
...
| `runAsUser` | `1000` | Fixed UID matching the Dockerfile `review_sentinel` user |
...
- `/tmp` — temporary files
- `/home/review_sentinel` — user home directory (uv cache, git config)

### Non-root Docker Images

The Dockerfile creates a dedicated `review_sentinel` user (UID 1000, GID 1000) and `chown`s the application and workspace directories to it. The image does **not** set `USER review_sentinel` because GitHub Actions container jobs require root access to host-mounted volumes. Instead, non-root execution is enforced at the Kubernetes layer via `runAsUser: 1000` and `runAsNonRoot: true` in the pod security context.
