...
- `review-sentinel serve` — starts the webhook server.
- `review-sentinel review owner/repo#42` — one-shot CLI review (supports `--dry-run`, `--platform`, `--prompt`).
- `review-sentinel ci {platform}` — CI mode review (GitHub Actions / GitLab CI). Uses the LLM provider API runner.
- `review-sentinel run-job` — internal: executes a single job in a K8s pod (invoked by `KubernetesRunner`).
...
1. Load repo-level `.review_sentinel/guidelines.md` (or a custom source — inline via `CODING_GUIDELINES`, file via `CODING_GUIDELINES_FILE`; see [docs/reference/env-vars.md](../docs/reference/env-vars.md#resolution-rules)). Defaults to empty.
2. Detect languages from changed file extensions.
3. Load per-language `.review_sentinel/languages/{lang}.md` (overrides bundled `review_sentinel/prompts/languages/{lang}.md`).
...
- `WORKSPACE_BASE_DIR` — workspace root (default: `/tmp/review-sentinel`).
