...
USAGE: str = """\
Usage: review-sentinel <command> [args]

Commands:
  serve              Start the webhook server
  review             One-shot CLI review (e.g. review-sentinel review owner/repo#42)
...
def main() -> None:
    ...
    if command == "review":
        from review_sentinel.commands.cli import cli_main
    ...
    if command == "ci":
        if len(sys.argv) < 3:
            print("Usage: review-sentinel ci <platform>", file=sys.stderr)
            sys.exit(1)

        from review_sentinel.commands.ci import run_ci_review
