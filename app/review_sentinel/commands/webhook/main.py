...
from review_sentinel.commands.webhook.helpers import acknowledge_event, extract_mention
...
from review_sentinel.commands.webhook.result import DispatchResult, DispatchStatus
...
from review_sentinel.config.settings import WebhookConfig
from review_sentinel.models import COMMENT_EVENT_TYPES
from review_sentinel.platforms import build_platforms
...
if TYPE_CHECKING:
    from review_sentinel.commands.webhook.jobs.runner.base import JobRunner
...
async def run_webhook_server() -> None:
    ...
    try:
        config: Config = load_config(require_webhook=True)
    except (OSError, ValueError) as exc:
        logger.error("Configuration error: %s", exc)
        sys.exit(1)

    platforms: dict[str, Platform] = build_platforms(config)
...
async def dispatch_lifecycle_event(
    ...
    return DispatchResult(status=DispatchStatus.ACCEPTED)
...
async def dispatch_comment_event(
    ...
    if not proceed:
        return DispatchResult(status=DispatchStatus.UNAUTHORIZED, http_status=403)
