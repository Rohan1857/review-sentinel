...
class KubernetesRunner:
    ...
    async def _execute(self, job: JobPayload) -> None:
        ...
        try:
            ...
            logger.info(
                "K8s Job for %s#%d completed with status: %s",
            ...
            logger.error(
                "K8s Job for %s#%d timed out after %ds",
                job.event.repo_full_name,
                job.event.pr_number,
                timeout_seconds,
            )
    ...
    def _build_job_spec(
        ...
        labels: dict[str, str] = {
            "app.kubernetes.io/name": "review-sentinel",
            "app.kubernetes.io/component": "job",
            "review-sentinel/platform": job.event.platform,
            "review-sentinel/repo": repo_slug,
            "review-sentinel/pr-number": str(job.event.pr_number),
        }

        generate_name: str = f"review-sentinel-job-{repo_slug}-{job.event.pr_number}-"
        ...
        container: dict[str, Any] = {
            ...
            "command": ["uv", "run", "--no-sync", "review-sentinel", "run-job"],
            "env": env_vars,
            "envFrom": env_from,
        }
        ...
        container["volumeMounts"] = [
            ...
            {"name": "home", "mountPath": "/home/review_sentinel"},
        ]
...
def build_job_channel_key(job: JobPayload) -> str:
    ...
    The key is derived from ``(platform, repo, pr_number)`` which
    uniquely identifies the serialized job slot. Since the job queue
    guarantees only one job runs per slot at a time, this key is safe
    to use without a UUID.
