...
from review_sentinel.commands.webhook.jobs.queue.asyncio import AsyncioJobQueue
from review_sentinel.commands.webhook.jobs.runner.process import ProcessRunner
from review_sentinel.commands.webhook.main import create_app
from review_sentinel.config import (
    CliAgentConfig,
    Config,
    ReviewerConfig,
    WebhookConfig,
...
from review_sentinel.platforms.github import GitHubPlatform
from review_sentinel.platforms.github.auth import GitHubAppAuth
...
def github_app_credentials() -> tuple[str, str, int]:
    ...
    app_id = os.environ.get("REVIEWSENTINELBOT_GITHUB_APP_ID", "")
    installation_id_raw = os.environ.get("REVIEWSENTINELBOT_GITHUB_INSTALLATION_ID", "")

    private_key = os.environ.get("REVIEWSENTINELBOT_GITHUB_APP_PRIVATE_KEY", "")

    if not private_key:
        key_path = os.environ.get("REVIEWSENTINELBOT_GITHUB_APP_PRIVATE_KEY_PATH", "")
    ...
    if not app_id or not private_key or not installation_id_raw:
        pytest.skip(
            "REVIEWSENTINELBOT_GITHUB_APP_ID, REVIEWSENTINELBOT_GITHUB_APP_PRIVATE_KEY "
            "(or _PATH), and REVIEWSENTINELBOT_GITHUB_INSTALLATION_ID are required"
        )
