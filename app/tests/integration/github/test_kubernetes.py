...
JOB_IMAGE = os.environ.get("K8S_IMAGE", "review-sentinel")
    ...
    return json.dumps(
        {
            ...
            "comment": {
                "id": 99999999,
                "body": "@review_sentinel-bot review this PR",
...
async def test_kubernetes_job_dispatch(
    ...
    for _ in range(JOB_CREATION_TIMEOUT // 2):
        stdout, _, returncode = await _kubectl(
            ...
            NAMESPACE,
            "-l",
            f"review-sentinel/platform=github,review-sentinel/pr-number={pr_info.number}",
            "-o",
            "json",
            check=False,
        )
    ...
    assert labels["review-sentinel/pr-number"] == str(pr_info.number)

    expected_repo_label = GITHUB_TEST_REPO.replace("/", "-")
    assert labels["review-sentinel/repo"] == expected_repo_label
    ...
    assert "review-sentinel-secrets" in secret_refs
