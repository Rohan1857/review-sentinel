...
from review_sentinel.commands.webhook.main import create_app, should_process_event
...
class TestGitHubWebhook:
    ...
    async def test_github_webhook_reviewer_mention(self, client, app):
        ...
        with patch(
            "review_sentinel.commands.webhook.main.acknowledge_event",
...
class TestMakeWebhookHandler:
    def test_make_webhook_handler_returns_callable(self, app):
        from review_sentinel.commands.webhook.main import _make_webhook_handler
...
class TestTitleTagFilter:
    ...
    def test_include_tag_in_title_accepts(self):
        ...
        event = self._lifecycle("test: webhook [review_sentinelbot]")
    ...
    def test_exclude_takes_priority_over_include(self):
        ...
        event = self._lifecycle("test: [review_sentinelbot] [skip]")

        assert should_process_event(event=event, filtering=filtering) is False

    def test_case_insensitive_matching(self):
        filtering = FilteringPolicy(
            pr_title_include_tags=frozenset(["reviewsentinelbot"]),
        )
        event = self._lifecycle("test: [ReviewSentinelBot] feature")
    ...
    def test_comment_event_with_pr_title_accepted(self):
        ...
        event = self._comment("test: [review_sentinelbot] feature")
