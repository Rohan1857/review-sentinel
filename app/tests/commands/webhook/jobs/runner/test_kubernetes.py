...
from review_sentinel.commands.webhook.jobs.payload import JobPayload
from review_sentinel.commands.webhook.jobs.queue.redis import RedisJobQueue
from review_sentinel.commands.webhook.jobs.runner.kubernetes import (
    KubernetesRunner,
    _slugify,
    build_job_channel_key,
    publish_job_completion,
)
from review_sentinel.config import KubernetesConfig
from review_sentinel.config.settings import RedisConfig
from review_sentinel.models import EventType
from review_sentinel.platforms.base import CommentEvent, PlatformName


def _make_config():
    return KubernetesConfig(
        namespace="review-sentinel",
        image="review-sentinel:dev",
        service_account="review_sentinel-sa",
...
class TestBuildJobChannelKey:
    def test_format(self):
        ...
        assert key == "nc:job:github:owner/repo:42"
...
class TestKubernetesRunnerBuildJobSpec:
    def test_spec_structure(self):
        ...
        assert spec["metadata"]["generateName"] == "review-sentinel-job-owner-repo-42-"
        assert spec["metadata"]["namespace"] == "review-sentinel"
    ...
    def test_labels(self):
        ...
        assert labels["app.kubernetes.io/name"] == "review-sentinel"
        assert labels["review-sentinel/platform"] == "github"
        assert labels["review-sentinel/pr-number"] == "42"
    ...
    def test_service_account(self):
        ...
        assert pod_spec["serviceAccountName"] == "review_sentinel-sa"
    ...
    def test_redis_url_always_forwarded(self):
        config = _make_config()
        redis = RedisConfig(url="redis://localhost:6379")
        runner = KubernetesRunner(
            config=config,
...
class TestKubernetesRunnerExecute:
    ...
    async def test_create_and_await_success(self):
        ...
        with (
            patch(
                "review_sentinel.commands.webhook.jobs.runner.kubernetes._read_service_account_token",
                return_value="test-token",
            ),
            patch(
                "review_sentinel.commands.webhook.jobs.runner.kubernetes.httpx.AsyncClient",
