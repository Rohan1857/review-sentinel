...
from review_sentinel.commands.cli.main import (
    _build_cli_parser,
    _parse_pr_ref,
    _print_review,
    _run_review,
...
from review_sentinel.models import AgentReview, ReviewFinding
from review_sentinel.review.reviewer import ReviewResult
...
class TestRunReview:
    ...
    async def test__run_review_dry_run_does_not_post(self):
        ...
        with patch(
            "review_sentinel.commands.cli.main.build_platform",
            return_value=mock_platform,
        ):
            with patch(
                "review_sentinel.commands.cli.main.review",
...
class TestCliMain:
    def test_cli_main_exits_nonzero_with_no_args(self):
        with patch.object(sys, "argv", ["review-sentinel"]):
            with patch("review_sentinel.commands.cli.main.setup_logging"):
    ...
    def test_cli_main_dispatches_review_command(self):
        with patch.object(sys, "argv", ["review-sentinel", "review", "owner/repo#1"]):
            with patch("review_sentinel.commands.cli.main.setup_logging"):
                with patch(
                    "review_sentinel.commands.cli.main._run_review",
    ...
    def test_cli_main_exits_with_review_return_code(self):
        with patch.object(sys, "argv", ["review-sentinel", "review", "bad#ref"]):
            with patch("review_sentinel.commands.cli.main.setup_logging"):
                with patch(
                    "review_sentinel.commands.cli.main._run_review",
