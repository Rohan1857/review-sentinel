...
from review_sentinel.agent.invoke import invoke_agent
...
class TestRunClaude:
    ...
    async def test_run_agent_returns_result(self):
        ...
        with patch("review_sentinel.agent.cli.runner.query", mock_query):
...
class TestPatchedParseMessage:
    def test_patched_parse_message_passes_through_valid_message(self):
        ...
        from review_sentinel.agent.cli.runner import (
        ...
        with patch(
            "review_sentinel.agent.cli.runner._original_parse_message"
        ) as mock_orig:
    ...
    def test_patched_parse_message_returns_system_message_on_parse_error(self):
        ...
        from review_sentinel.agent.cli.runner import _patched_parse_message
...
class TestRunAgentApiDispatch:
    ...
    async def test_run_agent_threads_prior_messages_to_api_runner(self):
        ...
        from review_sentinel.llm.messages import Message, TextBlock
        ...
        with (
            patch(
                "review_sentinel.agent.invoke.run_api_agent",
                side_effect=mock_run_api,
            ),
            patch(
                "review_sentinel.agent.invoke.create_provider",
...
class TestLogMessage:
    def test_log_message_does_nothing_when_debug_disabled(self):
        ...
        from review_sentinel.agent.cli.runner import _log_message

        message = MagicMock()

        with patch("review_sentinel.agent.cli.runner.logger") as mock_logger:
