...
from review_sentinel.llm.provider import MissingProviderError

pytest.importorskip("anthropic")

from review_sentinel.llm.anthropic import (
    AnthropicProvider,
    _to_api_messages,
    _to_api_tools,
    _to_llm_response,
...
class TestAnthropicProviderMissingSdk:
    ...
    def test_missing_provider_error_includes_install_instructions(self):
        error = MissingProviderError(
            provider="anthropic",
            library="anthropic",
            instruction='pip install "review-sentinel[anthropic]"',
        )

        assert "review-sentinel[anthropic]" in str(error)
