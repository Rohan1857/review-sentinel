...
from review_sentinel.config import CliAgentConfig, ReviewerConfig
...
from review_sentinel.platforms.base import CommentEvent, ExistingComment, PlatformName
from review_sentinel.review.output import FALLBACK_MESSAGE
from review_sentinel.review.prompts import (
    build_codebase_reviewer_prompt,
    build_reviewer_prompt,
    format_existing_comments,
)
from review_sentinel.review.reviewer import (
...
def _make_platform():
    from review_sentinel.platforms.base import PullRequestMetadata
...
class TestReviewerProcessComment:
    ...
    async def test_reviewer_calls_fetch_pr_diff(self):
        ...
        with patch(
            "review_sentinel.agent.invoke.run_cli_agent",
            ...
            with patch(
                "review_sentinel.workspace.setup.GitWorkspace",
            ) as mock_ws_class:
                mock_ws = MagicMock()
    ...
    async def test_reviewer_uses_resolve_coding_guidelines(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "review_sentinel.review.reviewer.resolve_guidelines",
    ...
    async def test_reviewer_repair_on_invalid_json(self):
        ...
        with (
            ...
            patch(
                "review_sentinel.review.output.invoke_agent",
                new_callable=AsyncMock,
            ) as mock_repair_run,
        ):
    ...
    async def test_reviewer_fetches_diff_comments_and_workspace_in_parallel(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "review_sentinel.review.reviewer.asyncio.gather",
...
class TestAgenticReviewer:
    ...
    async def test_review_uses_agentic_flow_for_api_agent(self):
        from review_sentinel.config import AgentRoleConfig, ApiAgentConfig
        ...
        with (
            patch(
                "review_sentinel.review.reviewer.create_provider",
                return_value=mock_provider,
            ),
            patch(
                "review_sentinel.review.reviewer.invoke_agent",
                new_callable=AsyncMock,
...
def _make_lifecycle_event():
    from review_sentinel.platforms.base import LifecycleEvent
...
class TestPrepareReviewContextIgnorePatterns:
    ...
    async def test_excluded_files_logged_at_warning(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="review_sentinel.review.reviewer"):
