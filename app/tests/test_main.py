...
from review_sentinel.main import main, setup_logging


class TestSetupLogging:
    def test_setup_logging_default_level_is_info(self):
        with patch.dict("os.environ", {}, clear=True):
            with patch("review_sentinel.main.logging.basicConfig") as mock_basic:
                setup_logging()
...
class TestMain:
    def test_main_dispatches_to_cli_main_when_review_arg(self):
        with patch.object(sys, "argv", ["review-sentinel", "review", "owner/repo#1"]):
            with patch("review_sentinel.commands.cli.cli_main") as mock_cli:
                main()

                mock_cli.assert_called_once()

    def test_main_serve_calls_asyncio_run(self):
        with patch.object(sys, "argv", ["review-sentinel", "serve"]):
            with patch("review_sentinel.main.setup_logging"):
                with patch(
                    "review_sentinel.commands.webhook.main.run_webhook_server",
                    new=MagicMock(),
                ):
                    with patch("review_sentinel.main.asyncio.run") as mock_run:
                        main()
        ...
        with patch.object(sys, "argv", ["review-sentinel", "serve"]):
            with patch("review_sentinel.main.setup_logging"):
                with patch(
                    ...
                    with patch(
                        "review_sentinel.main.asyncio.run",
    ...
    def test_main_serve_calls_setup_logging_before_asyncio_run(self):
        ...
        with patch.object(sys, "argv", ["review-sentinel", "serve"]):
            with patch(
                "review_sentinel.main.setup_logging",
    ...
    def test_main_no_args_exits_with_error(self):
        with patch.object(sys, "argv", ["review-sentinel"]):
    ...
    def test_main_unknown_command_exits_with_error(self):
        with patch.object(sys, "argv", ["review-sentinel", "bogus"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
    ...
    def test_main_ci_without_platform_exits_with_error(self):
        with patch.object(sys, "argv", ["review-sentinel", "ci"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
...
class TestRunWebhookServer:
    ...
    async def test_run_webhook_server_exits_on_config_error(self):
        with patch(
            "review_sentinel.commands.webhook.main.load_config",
            side_effect=ValueError("bad config"),
        ):
            with pytest.raises(SystemExit) as exc_info:
                from review_sentinel.commands.webhook.main import run_webhook_server
    ...
    async def test_run_webhook_server_exits_when_no_platforms(self):
        ...
        with patch(
            ...
            with patch(
                "review_sentinel.commands.webhook.main.build_platforms",
                return_value={},
            ):
                with pytest.raises(SystemExit) as exc_info:
                    from review_sentinel.commands.webhook.main import run_webhook_server
