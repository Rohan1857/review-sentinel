...
def source_files(cwd: str) -> list[Path]:
    """
    Return all non-test Python source files under app/review_sentinel/.
    ...
    source_dir = Path(cwd) / "app" / "review_sentinel"
...
def _run_hook() -> None:
    ...
    violation_list = "\n".join(all_violations)
    reason = (
        "Docstring violations detected in review_sentinel/. "
...
def _run_ci() -> None:
    ...
    Must be called from the repository root so that app/review_sentinel/ is resolvable.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Check docstring conventions in review_sentinel/.")
    parser.add_argument(
        "--ci",
        action="store_true",
