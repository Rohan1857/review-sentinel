...
pricing for all chat models from providers supported by review-sentinel,
and writes the result to ``app/review_sentinel/llm/data/pricing.json``.

Providers that use a LiteLLM key prefix (e.g. ``groq/``,
``together_ai/``) have the prefix stripped so the output keys match
the model IDs used by review-sentinel at runtime.
"""
